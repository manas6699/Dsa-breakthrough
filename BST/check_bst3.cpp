...
bool check_bst3 (binary_tree_node<int>* root ,int min = INT_MIN ,int max = INT_MAX)
{
    if (root == NULL)
    {
        return true;
    ...
    bool isrightok = check_bst3(root->right, root->data , max);
    bool isleftok = check_bst3(root->left ,min , max - 1);
