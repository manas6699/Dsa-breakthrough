...
int max_node(binary_tree_node<int> *root)
{
    ...
    int max = root->data;
    int l_max = max_node(root->left);
    int r_max = max_node(root->right);
    if (l_max > max)
    {
        max = l_max;
    }
...
int min_node(binary_tree_node<int> *root)
{
    if (root == NULL)
    {
        return INT_MIN;
    ...
    if (root->left != NULL)
    {
        l_min = min_node(root->left);
    }

    if (root->right != NULL)
    {
        r_min = min_node(root->right);
    }
...
//      4 5 6 7 8 9 10 -1 -1 -1 -1 -1 -1 -1 -1
...
int main()
{
    ...
    cout << "Maximum is : " << max_node(p1) << endl;
    cout << "Minimum is : " << min_node(p1) << endl;
